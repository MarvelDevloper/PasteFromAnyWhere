...
import { Save, ArrowLeft, Code2, Lock, Globe, AlertCircle, FileText } from '../components/Icons';
...
const EditPaste = () => {
      ...
      try {
        ...
        if (response.data?.success) {
          ...
          if (Array.isArray(fetchedPastes)) {
            target = fetchedPastes.find((p) => p._id === pasteId) || fetchedPastes[0];
          } else {
            target = fetchedPastes;
          }
        ...
        setError(err.response?.data?.msg || 'Failed to load paste details.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await pasteApi.update(pasteId, formData);
      showToast(response.data?.msg || 'Paste updated successfully!', 'success');
      navigate(`/paste/${pasteId}`);
    } catch (err) {
      setError(err.response?.data?.msg || 'Failed to update paste.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="main-content animate-fade-in" style={{ maxWidth: '900px' }}>
      <div style={{ marginBottom: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Link to={`/paste/${pasteId}`} className="btn btn-secondary" style={{ padding: '0.45rem 0.9rem', fontSize: '0.85rem' }}>
          <ArrowLeft size={16} />
      ...
      <div className="glass-card">
        <div style={{ marginBottom: '1.5rem' }}>
          <h1 style={{ fontSize: '1.6rem', fontWeight: 800, marginBottom: '0.3rem' }}>Edit Paste</h1>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>
            Modify title, syntax, privacy settings, or raw content.
          </p>
        </div>
          ...
          <>
            ...
            <form onSubmit={handleSubmit}>
              <div className="form-group">
                ...
                <div style={{ position: 'relative' }}>
                  <input
                    ...
                    placeholder="e.g. Updated Auth Middleware"
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem', marginTop: '1.25rem' }}>
                <Link to={`/paste/${pasteId}`} className="btn btn-secondary">
                  Cancel
                </Link>
